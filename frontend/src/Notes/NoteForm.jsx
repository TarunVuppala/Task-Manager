...
function NoteForm({ handleNotesOpen, noteAddDel, setNoteAddDel, note }) {
    const updateNote = useRef(false);
    if (note._id) {
        updateNote.current = true;
    }
    const { user } = useUser();
    ...
    return (
        <form className='flex flex-col p-10 h-screen gap-[21px] dark:text-white bg-transparent transition-all' onSubmit={updateNote.current ? handleUpdate : handleNotesForm}>
            <input
                type="text"
                id="heading"
                name="heading"
            ...
            <div className='flex flex-col justify-between gap-4'>
                <div className='flex flex-row justify-start items-center'>
                    <button type='submit' className='py-3 px-5 rounded-[20px] bg-[#F04D23] font-black text-white'>{updateNote.current?'Update Note':'Add Note'}</button>
